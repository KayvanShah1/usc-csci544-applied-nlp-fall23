...
def test_greedy_records_format(greedy):
    """This test checks if all the records in the greedy file have the attributes: index, sentence,
    labels"""
    with open(greedy) as f:
        predictions_json = json.load(f)
    ...
    for record in predictions_json:
        assert len(set(record.keys()).intersection({"index", "sentence", "labels"})) == 3, (
            "The items in your greedy.json file should have index, sentence, labels. No more, no"
            " less"
        )


def test_greedy_sentence_len(greedy, test):
    """This test checks if all the records in the greedy file have sentence length equal to the
    test file"""
    with open(greedy) as f:
        predictions_json = json.load(f)
...
def test_greedy_sentences_labels_equal_len(greedy):
    """This test checks if all the records in the greedy file have the sentence length equal to the
    label length"""
    with open(greedy) as f:
...
def test_greedy_sentence_integrity(greedy, test):
    ...
    for predictions_record, test_record in zip(predictions_json, test_json):
        for pred_word, test_word in zip(predictions_record["sentence"], test_record["sentence"]):
            assert (
                pred_word == test_word
            ), f"sentence {predictions_record['index']} in greedy.out has a word mismatch"
